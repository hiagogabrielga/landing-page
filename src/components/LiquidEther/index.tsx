...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const imageATexture = new THREE.TextureLoader().load(
      "/assets/Ayrton-Senna-n-capa.png",
    );
    const imageBTexture = new THREE.TextureLoader().load(
      "/assets/Ayrton-Senna-c-capa.png",
    );

    [imageATexture, imageBTexture].forEach((tex) => {
      tex.minFilter = THREE.LinearFilter;
      tex.magFilter = THREE.LinearFilter;
      tex.wrapS = THREE.ClampToEdgeWrapping;
      tex.wrapT = THREE.ClampToEdgeWrapping;
...
void main() {
  // Intensidade do fluido
  vec2 vel = texture2D(velocity, uv).xy;
  float mask = length(vel) * revealStrength;

  // Suaviza o efeito
  mask = smoothstep(0.05, 0.3, mask);
  mask = clamp(mask, 0.0, 1.0);

  vec4 base = texture2D(imageA, uv);
  vec4 reveal = texture2D(imageB, uv);

  // Mistura A + B usando o fluido como máscara
...
void main() {
  ...
  vec2 distortedUV = uv + vel * distortionStrength;
        ...
        this.output = new THREE.Mesh(
          ...
          new THREE.RawShaderMaterial({
            ...
            uniforms: {
              ...
              imageA: { value: imageATexture },
              imageB: { value: imageBTexture },
              revealStrength: { value: 1.8 },
            },
          }),
        );
